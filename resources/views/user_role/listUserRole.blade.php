              ...
              <div class="card-header">
                ...
                <div class="heading-elements">
                  <ul class="list-inline mb-0">
                    ...
                    <li><a class="btn btn-success text-white" href="{{route('userRole.create')}}">{{__('create')}}</a>
                    </li>
                    ...
                    <table class="table table-striped table-bordered default-ordering dataTable" id="DataTables_Table_1"
                      ...
                      <thead>
                        <tr role="row">
                          ...
                          <th class="sorting_desc" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1"
                            colspan="1" aria-label="Name: activate to sort column ascending" style="width: 69.4875px;"
                            aria-sort="descending">{{__('permissions')}}</th>
                          <th class="sorting_desc" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1"
                            colspan="1" aria-label="Name: activate to sort column ascending" style="width: 69.4875px;"
                            aria-sort="descending">{{__("created_at")}}</th>
                          ...
                          <td class="d-flex ">
                            ...
                            <form action="{{route('userRole.destroy',  $role->id)}}" method="post">
                              ...
                              <button type="submit" class="btn btn-danger confirm-button"><i class="la la-trash"
                                  title="{{__("delete")}}" data-toggle="tooltip" data-placement="top"></i></button>
                            </form>
                            @endcan
...
$('#DataTables_Table_1').on('click','.confirm-button', async function(event) {
        ...
        try {
                ...
                const firstConfirm = await Swal.fire({
                    title: "{{__('delete_question')}}",
                    text:"{{__('delete_warn')}}",
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonText: "{{__('deletey_yes')}}",
                    cancelButtonText: "{{__('deletey_no')}}"
                });
                ...
                if (firstConfirm.isConfirmed) {
                    ...
                    const secondConfirm = await Swal.fire({
                        title: "{{__('delete_question')}}",
                        input: 'text',
                        inputLabel: "{{__('delete_enter_confirm')}}",
                    ...
                    if (secondConfirm.isConfirmed && secondConfirm.value =='confirm') {
                        ...
                        Swal.fire({
                            title: "{{__('cancel')}}",
                            text: "{{__('delete_confirm_notvalid')}}",
                            icon: 'info'
                        });
                    }
                } else {
