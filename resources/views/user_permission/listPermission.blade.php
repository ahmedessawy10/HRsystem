              ...
              <div class="card-header">
                ...
                <div class="heading-elements">
                  <ul class="list-inline mb-0">
                    ...
                    <li>
                      <a class="btn btn-create" href="{{ route('permission.create') }}">
                    ...
                    <table class="table table-striped table-bordered default-ordering dataTable" id="DataTables_Table_1"
                      ...
                      <tbody>
                        ...
                        <tr role="row" class="odd">
                          ...
                          <td>{{ $premission->clear_name ?? '' }}</td>
                          <td>{{ \Carbon\Carbon::parse($premission->created_at)->format('d/m/Y') }}</td>
                          <td class="action-icons">
                            @can('permission update')
                            <a class="action-icon edit" href="{{ route('permission.edit', $premission->id) }}" title="{{ __('project.edit') }}">
                              <i class="la la-pencil"></i>
                            </a>
                            @endcan

                            @can('permission delete')
                            <form action="{{ route('permission.destroy', $premission->id) }}" method="post" style="display:inline;">
                              @csrf
                              @method('delete')
