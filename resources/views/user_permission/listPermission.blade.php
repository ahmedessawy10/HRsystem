          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h4 class="card-title"> {{__('project.list of permission')}} </h4>
                <a class="heading-elements-toggle"><i class="la la-ellipsis-v font-medium-3"></i></a>
                <div class="heading-elements">
                  <ul class="list-inline mb-0">
                    @can('permission create_and_view')
                    <li><a class="btn btn-success text-white"
                        href="{{route('permission.create')}}">{{__('project.create')}}</a></li>
                    @endcan
                  </ul>
                </div>
              </div>
                    ...
                    <table class="table table-striped table-bordered default-ordering dataTable" id="DataTables_Table_1"
                      ...
                      <tbody>
                        ...
                        <tr role="row" class="odd">
                          ...
                          <td class="">{{$premission->id}}</td>
                          <td class="">{{$premission->name}}</td>
                          <td class="">{{$premission->clear_name?? ''}}</td>
                          <td class="">{{ Carbon\Carbon::parse($premission->created_at)->format('d/m/Y')}}</td>



                          <td class="d-flex ">
                            @can('permission update')
                            <a class="btn btn-success mx-2" href="{{route('permission.edit',$premission->id)}}"><i
                                class="la la-pencil" title="{{__('project.edit')}}" data-toggle="tooltip"
                            ...
                            <form action="{{route('permission.destroy', $premission->id)}}" method="post">
                              @csrf
                              @method('delete')
